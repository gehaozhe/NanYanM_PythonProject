...
class Login:
    ...
    @classmethod
    def get_user_info_from_cookie(cls, cookie_content):
        """
        从cookie内容中提取用户信息
        """
        import re
        ...
        # 提取必要的cookie值
        sessdata = extract_cookie_value(cookie_content, "SESSDATA")
        dedeuserid = extract_cookie_value(cookie_content, "DedeUserID")
        dedeuserid_ckmd5 = extract_cookie_value(cookie_content, "DedeUserID__ckMd5")
        bili_jct = extract_cookie_value(cookie_content, "bili_jct")

        # 验证cookie是否完整
        if not all([sessdata, dedeuserid, dedeuserid_ckmd5, bili_jct]):
            return None
        ...
        try:
            data = cls.request_get(url, headers = headers).get("data")
            if data:
                # 清除旧的cookies并设置新的cookies
                RequestUtils.session.cookies.clear()
                ...
                for cookie in cookie_content.split(';'):
                    ...
                    if cookie:
                        ...
                        if len(parts) == 2:
                            key, value = parts
                            RequestUtils.session.cookies.set(key, value)
                ...
                info = {
                    ...
                    "bili_jct": bili_jct,
                    "login_expires": DateTime.get_timedelta(DateTime.now(), 365)
                }
                return info
        except Exception:
            return None
