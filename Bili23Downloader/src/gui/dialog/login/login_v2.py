...
class CookiePanel(Panel):
    def __init__(self, parent, login_dialog=None):
        self.parent: LoginDialog = login_dialog or parent
    ...
    def init_UI(self):
        ...
        self.cookie_login_btn = wx.StaticText(self, -1, _("Cookie登录"))
        self.cookie_login_btn.SetFont(font)
        self.cookie_login_btn.SetCursor(wx.Cursor(wx.CURSOR_HAND))
        self.cookie_login_btn.SetForegroundColour(wx.Colour(79, 165, 217))

        swicher_hbox = wx.BoxSizer(wx.HORIZONTAL)
        swicher_hbox.AddStretchSpacer()
        swicher_hbox.Add(self.cookie_login_btn, 0, wx.ALL, self.FromDIP(6))
        swicher_hbox.AddStretchSpacer()

        cookie_lab = wx.StaticText(self, -1, _("请输入Cookie"))
        self.cookie_text_ctrl = wx.TextCtrl(self, style=wx.TE_MULTILINE | wx.TE_PROCESS_ENTER, size=self.FromDIP((200, 100)))
        self.cookie_text_ctrl.SetHint(_("请从B站网页端复制完整Cookie内容"))
        ...
        cookie_vbox = wx.BoxSizer(wx.VERTICAL)
        cookie_vbox.AddStretchSpacer()
        cookie_vbox.Add(swicher_hbox, 0, wx.EXPAND)
        cookie_vbox.AddSpacer(self.FromDIP(6))
        cookie_vbox.Add(cookie_lab, 0, wx.ALL & (~wx.TOP) | wx.ALIGN_CENTER, self.FromDIP(6))
        cookie_vbox.Add(self.cookie_text_ctrl, 0, wx.ALL & (~wx.TOP) | wx.EXPAND, self.FromDIP(6))
        cookie_vbox.Add(login_hbox, 0, wx.EXPAND)
        cookie_vbox.AddStretchSpacer()

        self.SetSizerAndFit(cookie_vbox)
    ...
    def onLoginEVT(self, event):
        def worker():
            cookie_content = self.cookie_text_ctrl.GetValue().strip()
            if not cookie_content:
                wx.MessageDialog(self.parent, _("登录失败\n\nCookie不能为空"), _("警告"), wx.ICON_WARNING).ShowModal()
                return

            # 基本格式验证
            if "SESSDATA" not in cookie_content:
                wx.MessageDialog(self.parent, _("登录失败\n\nCookie格式不正确，缺少SESSDATA"), _("警告"), wx.ICON_WARNING).ShowModal()
                return

            try:
                info = Login.get_user_info_from_cookie(cookie_content)
                if info:
                    # 验证获取到的信息是否完整
                    required_fields = ["username", "SESSDATA", "DedeUserID", "DedeUserID__ckMd5", "bili_jct"]
                    if all(info.get(field) for field in required_fields):
                        Login.login(info)
                        self.parent.onSuccess(use_worker=False)
                    else:
                        wx.MessageDialog(self.parent, _("登录失败\n\n获取用户信息不完整"), _("警告"), wx.ICON_WARNING).ShowModal()
                else:
                    wx.MessageDialog(self.parent, _("登录失败\n\nCookie无效或已过期\n\n请从B站网页端复制完整Cookie并确保未过期"), _("警告"), wx.ICON_WARNING).ShowModal()
            except Exception as e:
                wx.MessageDialog(self.parent, _("登录失败\n\n处理Cookie时发生错误: %s") % str(e), _("警告"), wx.ICON_WARNING).ShowModal()
...
class LoginDialog(Dialog):
    ...
    def init_UI(self):
        ...
        # 右侧：登录方式选择（短信登录和Cookie登录）
        self.right_panel = Panel(self)
        
        # 创建右侧面板的标签页
        self.right_notebook = wx.Notebook(self.right_panel)
        
        # 创建短信登录和Cookie登录面板，传递self作为login_dialog参数
        self.sms_panel = SMSPanel(self.right_notebook, login_dialog=self)
        self.cookie_panel = CookiePanel(self.right_notebook, login_dialog=self)
        
        # 添加标签页
        self.right_notebook.AddPage(self.sms_panel, _("短信登录"))
        self.right_notebook.AddPage(self.cookie_panel, _("Cookie登录"))
        
        # 右侧面板布局
        right_vbox = wx.BoxSizer(wx.VERTICAL)
        right_vbox.Add(self.right_notebook, 1, wx.EXPAND)
        self.right_panel.SetSizer(right_vbox)
    ...
    def onCloseEVT(self, event: wx.CloseEvent):
        ...
        if hasattr(self.sms_panel, 'timer') and self.sms_panel.timer:
            self.sms_panel.timer.Stop()
